...
const FilePicker = props => {
  ...
  return (
    <>
      <label
        id={`${props.id || 'file-upload'}-label`}
        className={`custom-file-label btn btn-outline-primary px-5 ${props.labelClassName}`}
      ...
      <Tooltip
        ...
        target={`${props.id || 'file-upload'}-label`}
      ...
      <input
        ...
        type="file"
        className={`custom-file-input invisible ${props.className}`}
        onChange={() => filePicked()}
        accept={`${props.isPictureUpload ? '.png' : '.txt'}${props.accept ||
          ''}`}
      />
    </>
  );
